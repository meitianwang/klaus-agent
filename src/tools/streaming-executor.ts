...
//
// Fully aligned with claude-code's StreamingToolExecutor:
//   - Constructor takes (toolDefinitions, canUseTool, toolUseContext)
//   - Consumes runToolUse(toolUse, assistantMessage, canUseTool, toolUseContext)
//   - Yields MessageUpdate { message?, newContext? }
//   - siblingAbortController for cascading Bash errors
//   - Progress messages via message type detection
//   - contextModifiers accumulated from non-concurrent tool results
//   - interruptBehavior support
//   - getUpdatedContext() for accumulated context modifications
...
type TrackedTool = {
  ...
  contextModifiers?: Array<(context: ToolExecutionContext) => ToolExecutionContext>;
};
...
export class StreamingToolExecutor {
  ...
  private toolUseContext: ToolExecutionContext;
  ...
  // Aborting this does NOT abort the parent — the agent loop won't end the turn.
  ...
  constructor(
    private readonly toolDefinitions: readonly AgentTool[],
  ...
  private createSyntheticErrorMessage(
    ...
    if (reason === "user_interrupted") {
      return createToolResultMessage(
        toolUseId,
        withMemoryCorrectionHint(REJECT_MESSAGE),
        true,
        {
  ...
  private async executeTool(tool: TrackedTool): Promise<void> {
    ...
    const contextModifiers: Array<(context: ToolExecutionContext) => ToolExecutionContext> = [];
    ...
    const collectResults = async () => {
      ...
      // — that abort must bubble up to the parent controller so the agent
      // loop's post-tool abort check ends the turn.
      ...
      for await (const update of generator) {
        ...
        // Detect error results by inspecting message content
        // SDK uses flat message format: msg.role === 'user', msg.content is array
        const isErrorResult =
          update.message.role === "user" &&
          Array.isArray(update.message.content) &&
          (update.message.content as any[]).some(
            (_: any) => _.type === "tool_result" && _.is_error === true,
          );
        ...
        if (update.message) {
          ...
          if ((update.message as any).type === "progress") {
  ...
  *getCompletedResults(): Generator<MessageUpdate, void> {
   ...
   */
  getUpdatedContext(): ToolExecutionContext {
